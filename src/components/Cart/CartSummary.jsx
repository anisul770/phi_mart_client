...
const CartSummary = ({ totalPrice, itemCount, cart_id }) => {
  ...
  const createOrder = async () => {
    try {
      const order = await authApiClient.post("/orders/", { cart_id: cart_id });
      if (order.status === 201) {
        deleteCart();
        alert(`<SuccessAlert successMsg={"Order Created Successfully"} />`);
      }
      console.log(order);
    } catch (error) {
      console.log(error);
