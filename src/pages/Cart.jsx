...
const Cart = () => {
  ...
  const handleUpdateQuantity = async (itemId, newQuantity) => {
    ...
    setLocalCart((prevLocalCart) => {
      const updatedItems = prevLocalCart.items.map(item => item.id === itemId ? { ...item, quantity: newQuantity, total_price: item.product.price * newQuantity, } : item);
      return {
        ...prevLocalCart, items: updatedItems,
  ...
  return (
    <div className='container mx-auto px-4 py-8'>
      <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
        ...
        <div className=''>
          <CartSummary totalPrice={localCart.total_price} itemCount={localCart.items.length} cart_id={cartId}/>
        </div>
      </div>
    </div>
  );
